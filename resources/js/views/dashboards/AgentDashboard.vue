<template>
    <AsyncPage>
        <MDBContainer class="performance-tracker-view mb-5">
            <h1 class="mb-2 mb-4">Agent Dashboard</h1>
            <SmartSelectAjax v-if="authStore.hasAccessToArea('ACCESS_AREA_ADD_EDIT_AGENTS')" label="Agent" ajax-url="options/dialer_agents" v-model:selected="form.agent_id" :show-required="true"/>
            ...
            <MDBTabs v-model="activeTab" v-if="form.agent_id">
                ...
                <MDBTabNav pills justify tabsClasses="mb-3 text-center flex-md-row flex-column">
                    ...
                    <MDBTabItem tag="button" :wrap="false" tabId="performance-tracker" class="flex-fill">Performance Tracker</MDBTabItem>
                    <MDBTabItem tag="button" :wrap="false" tabId="write-ups" class="flex-fill">Write-Ups</MDBTabItem>
                    <MDBTabItem tag="button" :wrap="false" tabId="pips" class="flex-fill">PIPs</MDBTabItem>
                ...
                <MDBTabContent>
                    ...
                    <MDBTabPane tabId="performance-tracker">
                    ...
                    <MDBTabPane tabId="pips">
                        <PipList
                            :agent-id="form.agent_id"
                            :show-add-button="authStore.hasAccessToArea('ACCESS_AREA_MENU_PEOPLE_PIPS')"
                            :key="form.agent_id"
                            :embedded="true"
                        ></PipList>
                    </MDBTabPane>
