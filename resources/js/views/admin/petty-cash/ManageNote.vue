<template>
    <MDBModal
        id="noteModal"
        tabindex="-1"
        labelledby="noteModalLabel"
        v-model="showModalLocal"
        class="modal-input-spacing"
        staticBackdrop
    >
        <MDBModalHeader>
            <MDBModalTitle id="noteModalLabel">{{ modalTitle }}</MDBModalTitle>
        </MDBModalHeader>
        <MDBModalBody>
            <Suspense>
                <template #default>
                    <div>
                        <MDBInput label="Note" v-model="modalValuesLocal.note"/>
                    </div>
                </template>
                <template #fallback>
...
const modalTitle = computed(() => {
    return (props.modalValues.id ? 'Edit' : 'Add') + ' Note';
})
...
const save = () => {
    ...
    if (modalValuesLocal.value.id) {
        apiClient.patch(`petty-cash-notes/manage/${modalValuesLocal.value.id}`, payload)
            .then(({data}) => {
        ...
        apiClient.post(`petty-cash-notes/manage`, payload)
            .then(({data}) => {
                showModalLocal.value = false;
                if (data.id) {
                    emit('update:id', data.id);
