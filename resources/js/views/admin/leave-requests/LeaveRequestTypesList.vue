<template>
    <AbstractList
        ajax-url="leave-request-types/manage"
        title-singular="Leave Request Type"
        title-plural="Leave Request Types"
        :form="form"
    ...
    >
        ...
        <template v-slot:edit-modal>
            <MDBAlert v-if="form.id" color="warning" static class="mb-4"><i class="fas fa-exclamation-triangle me-3"></i> Changing this value will update the type for all leave requests already on file.</MDBAlert>

            <MDBInput label="Name" v-model="form.name" :maxlength=255 :required="true"/>

            <MDBInput label="Slug" v-model="form.slug" :maxlength=255 :required="true"/>
