    ...
    >
        ...
        <MDBModalBody>
            <Suspense>
                <template #default>
                    <div>
                        <MDBAlert v-if="modalValuesLocal.id" color="warning" static class="mb-4"><i class="fas fa-exclamation-triangle me-3"></i> It is recommended to add a new write-up reason since this existing reason may have already been set on Agents.</MDBAlert>
...
const modalTitle = computed(() => {
    return props.modalValues.id ? `Edit Write-Up Reason - ID ${props.modalValues.id}` : `Add a Write-Up Reason`;
})
...
const save = () => {
    ...
    if (modalValuesLocal.value.id) {
        apiClient.patch(`writeup-reasons/manage/${modalValuesLocal.value.id}`, payload)
            .then(({data}) => {
        ...
        apiClient.post(`writeup-reasons/manage`, payload)
            .then(({data}) => {
                showModalLocal.value = false;
                emit("reload");
            }).catch(error => {
