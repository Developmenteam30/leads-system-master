<template>
    ...
    >
        ...
        <template v-else>
            <MDBModalHeader>
                <MDBModalTitle :id="`label-${modalId}`">Delete {{ titleSingular }} - ID {{ props.form.id }}</MDBModalTitle>
            </MDBModalHeader>
            <MDBModalBody>
                <MDBAlert color="danger" static><i class="fas fa-bomb me-3"></i> Are you sure you wish to delete this record?</MDBAlert>
                <Suspense>
                    <template #default>
                        <slot></slot>
                    </template>
            ...
            <MDBModalFooter>
                ...
                <MDBBtn color="danger" @click="save">Delete
...
const save = () => {
    ...
    apiClient.delete(`${props.ajaxUrl}/${props.form.id}`, payload)
        .then(({data}) => {
