...
class LeaveRequestTypesController extends BaseController
{
    /**
     * Load an agent
     *
    ...
    public function index(Request $request, $agent_id = null)
    {
        ...
        AuditLog::createFromRequest($request, 'DIALER-LEAVE-REQUEST-TYPES:LIST', [
            'search' => $request->input('search'),
            'include_archived' => $request->input('include_archived'),
        ]);

        $items = DialerLeaveRequestType::query()
            ->select([
                'id',
                'name',
                'slug',
        ...
        return DataTableFields::displayOrExport($datatable, $allow_list, $request, "Leave Request Types.xlsx");
    }
    ...
    {
        if (!empty($item_id)) {
            $item = DialerLeaveRequestType::withTrashed()->find($item_id);
            if (!$item) {
                return ErrorResponse::json('Leave request type not found', 400);
            }
        } else {
            $item = new DialerLeaveRequestType();
        }
        ...
        ApiJsonValidator::validate($request->all(), [
            'name' => [
                ...
                'string',
                'max:255',
                !empty($item) ? Rule::unique('dialer_leave_request_types')->ignore($item) : Rule::unique('dialer_leave_request_types'),
            ],
            'slug' => [
                'bail',
                'required',
            ...
            if ($item->exists) {
                if ($request->boolean('isArchived')) {
                    if ($item->entries->count()) {
                        return ErrorResponse::json("This leave request type is in use by {$item->entries->count()} leave ".Str::plural('request', $item->entries->count()).".", 400);
                    }
                    $item->delete();
                } else {
                    $item->restore();
    ...
    public function delete(Request $request, $item_id = null)
    {
        ApiJsonValidator::validate($request->route()->parameters(), [
            'id' => 'required|bail|exists:dialer_leave_request_types,id',
        ]);
